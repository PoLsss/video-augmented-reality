...
models = []
# modelPipe = cv2.imread(os.path.join(dir_name, 'E:\TaiLieu\HK2_N3\ThiGiac\Project\img\cat.png'), 0)
modelPipe = cv2.imread(os.path.join(dir_name, 'references/meo.jpg'), 0)
modelPipe = cv2.resize(modelPipe,None,fx=resizeFactor,fy=resizeFactor, interpolation = cv2.INTER_AREA)
models.append(modelPipe)
# modelCafe = cv2.imread(os.path.join(dir_name, 'E:\TaiLieu\HK2_N3\ThiGiac\Project\img\golila.png'), 0)
modelCafe = cv2.imread(os.path.join(dir_name, 'references/c1.jpg'), 0)
...
u = 'http://10.4.105.218:8080/video'
...
while True:
    ...
    cv2.imshow("Webcam", originalFrame)
    cv2.imshow("AR", frame)
